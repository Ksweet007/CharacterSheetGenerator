define('services/CharacterCreatorAPI', function(require) {
	var _i = {
		ko: require('knockout'),
		$: require('jquery')
	};
	...
	function DBApiCls() {
		...
		if ("indexedDB" in window) {
			console.log("Your Browser does not support IndexedDB storage, so this site will not work.");

            var openRequest = indexedDB.open("CharacterCreator", 1);

            openRequest.onupgradeneeded = function(e) {
                self.db = createDatabase(e);
                createTable(self.db);
            }
    ...
    //config{type,data,databasename,tablename}
	DBApiCls.prototype.database = function(config) {
        config = {type:'POST'};
        if(true){
	...
	DBApiCls.prototype.createDatabase = function(e) {
		return e.target.result;
	};

	DBApiCls.prototype.CreateClassTable = function() {
		...
		if (!database.objectStoreNames.contains(tablename)) {
			...
			objectStore.createIndex("powersource", "powersource", {
				unique: false
			})
		}
	};
    DBApiCls.prototype.addObject = function(database, table, objtosave) {
	...
	DBApiCls.prototype.deleteDatabase = function(database) {
	...
	DBApiCls.prototype.getObj = function(database, table) {
	...
	DBApiCls.prototype.getList = function(database, table) {
		var transaction = database.transaction([table], "readonly");
		var objectStore = transaction.objectStore(table);
	...
	DBApiCls.prototype.search = function() {
