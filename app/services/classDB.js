define('services/classDB', function(require) {
	var _i = {
		ko: require('knockout'),
		$: require('jquery'),
		dbutils: require('services/databaseutils')
	};

	function ClassDbCls() {
		var self = this;
		
		if (_i.dbutils.isCompatable) {
			var openRequest = indexedDB.open("charcreator", 1);
			openRequest.onupgradeneeded = function(e) {
				self.db = _i._db.createDatabase(e);
				_i._db.createTable(self.db);
			}
	...
	ClassDbCls.prototype.createIndex = function(objectStore) {
		objectStore.createIndex("name", "name", {unique: true});
		objectStore.createIndex("powersource", "powersource", {	unique: false})
	};

	ClassDbCls.prototype.createTable = function(database) {
	...
	ClassDbCls.prototype.createDatabase = function(e) {
		var thisDB = e.target.result;
		return thisDB;
	};

	ClassDbCls.prototype.deleteDatabase = function(database) {
	...
	ClassDbCls.prototype.addObject = function(database, table, objtosave) {
	...
	ClassDbCls.prototype.getObj = function(database, table) {
	...
	ClassDbCls.prototype.getList = function(database, table) {
		var transaction = database.transaction([table], "readonly");
		var objectStore = transaction.objectStore(table);
	...
	ClassDbCls.prototype.search = function(){
